...
class CrazyRumble : public RocketGameMode
{
    private:
        ...
        void onGiveItem(ActorWrapper dispenser);
        void setRumbleOptions(CarWrapper car);
    public:
        CrazyRumble(RocketPlugin* rp) : RocketGameMode(rp) { _typeid = std::make_shared<std::type_index>(typeid(*this)); }
        void updateRumbleOptions();
        void setItemGiveRate(int newMaxTimeTillItem);
        void setItemValues(float newForceMultiplier = 1.0f, float newRangeMultiplier = 1.0f, float newDurationMultiplier = 1.0f);
