...
// A crazy rumble customizer gamemode for Rocket Plugin.
//
// Author:       Stanbroek
...
/// <summary>Updates the rumble options every time a player receives an item.</summary>
/// <remarks>Gets called post 'Function TAGame.PlayerItemDispenser_TA.GiveItem'.</remarks>
/// <param name='dispenser'><see cref="ActorWrapper"/> instance of the player</param>
/// <param name='params'>Delay since last update</param>
void CrazyRumble::onGiveItem(ActorWrapper)
{
    cvarManager->error_log("function not implemented.");
}


/// <summary>Sets the current rumble options for the given <see cref="car"/>.</summary>
/// <param name='car'>car to set the rumble options for</param>
void CrazyRumble::setRumbleOptions(CarWrapper car) {
    if (car.IsNull()) {
        return;
    }
    ...
    if (!rumbleItem.IsNull()) {
        ...
        if (rumbleItemName == "BallMagnet") {
            ...
            cvarManager->log("Unknown item: '" + rumbleItemName + "', please report to mod author.");
        }
...
void CrazyRumble::updateRumbleOptions()
{
...
/// <param name='maxTimeTillItem'>Maximum time till the next rumble item</param>
void CrazyRumble::setItemGiveRate(int)
{
    cvarManager->error_log("function not implemented.");
}
...
/// <summary>Resets the rumble item values, with an optional multiplier.</summary>
/// <param name='forceMultiplier'>Multiplies the force</param>
/// <param name='rangeMultiplier'>Multiplies the range</param>
/// <param name='durationMultiplier'>Multiplies the duration</param>
void CrazyRumble::setItemValues(float newForceMultiplier, float newRangeMultiplier, float newDurationMultiplier)
{
    ...
    // Plunger - RumblePickupComponentWrapper::TargetedPickup::SpringPickup::BallLassoPickup::BattarangPickup
...
void CrazyRumble::updateCars(bool update) {
    if (update) {
...
void CrazyRumble::RenderOptions()
{
    ...
    if (ImGui::SliderInt("till next item", &maxTimeTillItem, 0, 20, "%d seconds")) {
        gameWrapper->Execute([this, newMaxTimeTillItem = maxTimeTillItem](GameWrapper*) {
    ...
    if (ImGui::CollapsingHeader("Boot")) {
        ...
        updateCars(ImGui::SliderFloat("Boot Relative Force Normal Direction", &bootRelativeForceNormalDirection, -10.0f, 10.0f));
    ...
    if (ImGui::CollapsingHeader("Freezer")) {
        ...
        updateCars(ImGui::SliderFloat("Freezer Stop Momentum Percentage", &freezerStopMomentumPercentage, 0.0f, 100.0f));
    ...
    if (ImGui::CollapsingHeader("Grapplinghook")) {
        updateCars(ImGui::SliderFloat("Grapplinghook Impulse", &grapplinghookImpulse, 0.0f, 1000000.0f));
        updateCars(ImGui::SliderFloat("Grapplinghook Force", &grapplinghookForce, 0.0f, 1000000.0f));
        updateCars(ImGui::SliderFloat("Grapplinghook Max Rope Length", &grapplinghookMaxRopeLength, 0.0f, 10000.0f));
        updateCars(ImGui::SliderFloat("Grapplinghook Prediction Speed", &grapplinghookPredictionSpeed, 0.0f, 10000.0f));
        updateCars(ImGui::Checkbox("Grapplinghook Can Target Ball", &grapplinghookCanTargetBall));
        updateCars(ImGui::Checkbox("Grapplinghook Can Target Cars", &grapplinghookCanTargetCars));
        updateCars(ImGui::Checkbox("Grapplinghook Can Target Enemy Cars", &grapplinghookCanTargetEnemyCars));
        updateCars(ImGui::Checkbox("Grapplinghook Can Target Team Cars", &grapplinghookCanTargetTeamCars));
        updateCars(ImGui::SliderFloat("Grapplinghook Range", &grapplinghookRange, 0.0f, 50000.0f));
        updateCars(ImGui::SliderFloat("Grapplinghook Activation Duration", &grapplinghookActivationDuration, 0.0f, 100.0f));
    }
    if (ImGui::CollapsingHeader("Haymaker")) {
        ...
        updateCars(ImGui::SliderFloat("Haymaker Relative Force Normal Direction", &haymakerRelativeForceNormalDirection, -10.0f, 10.0f));
        ...
        updateCars(ImGui::SliderFloat("Haymaker Welded Vertical Force", &haymakerWeldedVerticalForce, 0.0f, 1000000.0f));
    ...
    if (ImGui::CollapsingHeader("Plunger")) {
        ...
        updateCars(ImGui::SliderFloat("Plunger Relative Force Normal Direction", &plungerRelativeForceNormalDirection, -10.0f, 10.0f));
    ...
    if (ImGui::CollapsingHeader("Haunted")) {
        ...
        updateCars(ImGui::SliderFloat("Haunted Ball Beam Activation Duration", &hauntedBallBeamActivationDuration, 0.0f, 100.0f));
    }
    if (ImGui::CollapsingHeader("Rugby")) {
        updateCars(ImGui::SliderFloat("Rugby Spikes Activation Duration", &rugbySpikesActivationDuration, 0.0f, 100.0f));
    }
}
...
void CrazyRumble::Activate(bool active)
{
    if (active && !isActive) {
        HookEventWithCallerPost<ActorWrapper>("Function TAGame.PlayerItemDispenser_TA.GiveItem", std::bind(&CrazyRumble::onGiveItem, this, std::placeholders::_1));
