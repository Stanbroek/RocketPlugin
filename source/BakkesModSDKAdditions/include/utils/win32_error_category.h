...
namespace detail
{
    ...
    public:
        ...
        _NODISCARD std::string message(int errVal) const override final
        {
            ...
            return std::string(error, len) + " (" + to_hex(errVal) + ")";
        }
    };
}
...
{
    return std::error_code(static_cast<int>(winErrorCode.Error), win32_error_category());
}

/// Create an error_code from a Windows error code.
inline std::error_code make_win32_error_code(DWORD errVal)
{
