...
SOCKET GetBoundSocket(const u_short port, const std::string& localIP = "0.0.0.0")
{
    ...
    if (sendSocket == INVALID_SOCKET) {
        ERROR_LOG("failed to create socket: " + make_winsock_error_code().message());
    ...
    if (bind(sendSocket, reinterpret_cast<sockaddr*>(&addrSrc), sizeof addrSrc) == SOCKET_ERROR) {
        ERROR_LOG("failed to bind socket: " + make_winsock_error_code().message());
        closesocket(sendSocket);
        return INVALID_SOCKET;
    }
    ...
    if (setsockopt(sendSocket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char*>(&bOptVal), bOptLen) == SOCKET_ERROR) {
        ERROR_LOG("failed set SO_REUSEADDR: " + make_winsock_error_code().message());
        closesocket(sendSocket);
        return INVALID_SOCKET;
    }
                    ...
                    const bool changePort, StunResponse* resp)
{
    ...
    TRACE_LOG("sending buf[{}]: " + quote(strBuf), sendBufLen);
    for (int i = 0; i < 3; i++) {
        if (sendto(sock, sendBuf, static_cast<int>(sendBufLen), 0, &addrDest, sizeof addrDest) == SOCKET_ERROR) {
            ERROR_LOG("failed to send data: " + make_winsock_error_code().message());
            return SOCKET_ERROR;
        }
        ...
        if (iResult == SOCKET_ERROR) {
            ERROR_LOG("failed to get socket status: " + make_winsock_error_code().message());
            return SOCKET_ERROR;
        }
        if (iResult != 0) {
            break;
    ...
    if (recvfrom(sock, recvBuf, static_cast<int>(recvBufLen), NULL, &addrRetDest, &addrRetDestSize) == SOCKET_ERROR) {
        ERROR_LOG("failed to receive data: " + make_winsock_error_code().message());
        return SOCKET_ERROR;
    }
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            if (getaddrinfo(ip.c_str(), port.c_str(), &hints, &result) != 0) {
                ERROR_LOG("failed to translate " + quote(ip + ":" + port) + ", " + make_winsock_error_code().message());
                continue;
            }
...
void P2PHost::findNATType(unsigned short port, bool threaded)
{
    ...
    if (iResult != 0) {
        ...
        ERROR_LOG("failed to initiate winsock: " + lastError.message());
        return;
    }
    ...
    for (sockaddr stunServer : ParseStunServers(STUN_SERVICES_FILE_PATH)) {
        ...
        natType = NATType::NAT_ERROR;
        lastError = make_winsock_error_code();
        LOG("Failed to connect to " + quote(stunServerAddr) + ": " + (lastError ? lastError.message() : "Connection timed out."));
    }
    if (!responded) {
    ...
    StunResponse resp2;
    TRACE_LOG("sending second stun request to " + quote(stunServerAddr) + " which IP change request");
    ...
    if (resp1.Addr.IP == resp3.Addr.IP && resp1.Addr.Port == resp3.Addr.Port) {
        ...
        StunResponse resp4;
        TRACE_LOG("sending fourth stun request to " + quote(stunServerAddr) + " which port change request");
...
void P2PHost::punchPort(const std::string& ip, unsigned short port, const bool threaded)
{
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        ERROR_LOG("failed to initiate winsock: " + make_winsock_error_code().message());
        return;
    }
